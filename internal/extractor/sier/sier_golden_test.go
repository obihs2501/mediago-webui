...
func TestExtractMock(t *testing.T) {
	...
	installMockTransport(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Host == "www.sieredu.com" && r.Method == http.MethodPost && r.URL.Path == "/web/uc/course/myCourse":
			writeFixture(t, w, fixtures, "course_list")
		case r.Host == "www.sieredu.com" && r.Method == http.MethodPost && r.URL.Path == "/web/course/queryPlanList":
			writeFixture(t, w, fixtures, "plan")
		case r.Host == "www.sieredu.com" && r.Method == http.MethodPost && r.URL.Path == "/web/course/catalog/getCourseCatalogDetail":
			writeFixture(t, w, fixtures, "catalog")
		default:
	...
	ext, err := extractor.Match("https://www.sieredu.com/course/1001?courseId=1001")
	if err != nil {
		t.Fatal(err)
	}
	info, err := ext.Extract("https://www.sieredu.com/course/1001?courseId=1001", &extractor.ExtractOpts{Cookies: jar})
	if err != nil {
		t.Fatal(err)
	}
	if info == nil {
	...
	if !strings.Contains(got, "cdn.example.com/sier.mp4") {
