...
func assertGoldenOutcome(t *testing.T, media *extractor.MediaInfo, err error) {
	...
	if err != nil {
		...
		allowed := []string{"xueersi", "login", "cookie", "auth", "blocked", "rejected", "cannot parse", "parse", "invalid character", "no playable", "no media", "empty", "failed", "requires", "required", "not found", "missing", "token"}
	...
	if media.Site != "xueersi" {
		t.Fatalf("Site = %q, want xueersi", media.Site)
	}
...
func TestExtractMock(t *testing.T) {
	...
	media, err := (&Xueersi{}).Extract("https://www.xueersi.com/course-detail/1001?courseId=1001&stuCouId=2001&couType=1", &extractor.ExtractOpts{Cookies: jar})
	...
	if !strings.Contains(got, "https://media.example.com/xueersi/lesson-1.m3u8") {
