...
func TestExtractMock(t *testing.T) {
	...
	installMockTransport(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Host == "course-api-online.speiyou.com" && r.Method == http.MethodGet && r.URL.Path == "/course/v1/student/course/subject-list":
			writeFixture(t, w, fixtures, "subject")
		case r.Host == "course-api-online.speiyou.com" && r.Method == http.MethodGet && r.URL.Path == "/course/v1/student/live/list":
			writeFixture(t, w, fixtures, "live_list")
		case r.Host == "course-api-online.speiyou.com" && r.Method == http.MethodGet && r.URL.Path == "/course/v1/student/course/list":
			writeFixture(t, w, fixtures, "course_list")
		case r.Host == "classroom-api-online.speiyou.com" && r.Method == http.MethodGet && r.URL.Path == "/classroom/basic/v2/init/auth":
			writeFixture(t, w, fixtures, "auth")
		case r.Host == "classroom-api-online.speiyou.com" && r.Method == http.MethodGet && r.URL.Path == "/playback/v1/video/init":
			writeFixture(t, w, fixtures, "video")
		default:
	...
	ext, err := extractor.Match("https://www.speiyou.com/course/1001?stuId=7001&stdSubject=math")
	if err != nil {
		t.Fatal(err)
	}
	info, err := ext.Extract("https://www.speiyou.com/course/1001?stuId=7001&stdSubject=math", &extractor.ExtractOpts{Cookies: jar})
	if err != nil {
		t.Fatal(err)
	}
	if info == nil {
