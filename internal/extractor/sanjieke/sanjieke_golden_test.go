...
func TestExtractMock(t *testing.T) {
	...
	installMockTransport(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Host == "web-api.sanjieke.cn" && r.Method == http.MethodGet && strings.HasSuffix(r.URL.Path, "/info"):
			writeFixture(t, w, fixtures, "study_info")
		case r.Host == "web-api.sanjieke.cn" && r.Method == http.MethodGet && strings.HasSuffix(r.URL.Path, "/content/tree"):
			writeFixture(t, w, fixtures, "tree")
		case r.Host == "web-api.sanjieke.cn" && r.Method == http.MethodGet && strings.Contains(r.URL.Path, "/content/node-1"):
			writeFixture(t, w, fixtures, "section")
		default:
	...
	ext, err := extractor.Match("https://study.sanjieke.cn/course/1001/2001")
	if err != nil {
		t.Fatal(err)
	}
	info, err := ext.Extract("https://study.sanjieke.cn/course/1001/2001", &extractor.ExtractOpts{Cookies: jar})
	if err != nil {
		t.Fatal(err)
	}
	if info == nil {
	...
	if info.Site != "sanjieke" {
		t.Fatalf("site=%q", info.Site)
	}
	got := firstPlayableURL(info)
	if !strings.Contains(got, "cdn.example.com/sanjieke.m3u8") {
