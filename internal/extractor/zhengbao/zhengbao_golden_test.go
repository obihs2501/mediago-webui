...
func assertGoldenOutcome(t *testing.T, media *extractor.MediaInfo, err error) {
	...
	if err != nil {
		...
		allowed := []string{"zhengbao", "login", "cookie", "auth", "blocked", "rejected", "cannot parse", "parse", "invalid character", "no playable", "no media", "empty", "failed", "requires", "required", "not found", "missing", "token"}
	...
	if media.Site != "zhengbao" {
		t.Fatalf("Site = %q, want zhengbao", media.Site)
	}
...
func TestExtractMock(t *testing.T) {
	...
	media, err := (&Zhengbao{}).Extract("https://www.chinaacc.com/course?courseIds=1001&cwareIDs=2001&identity=abc", &extractor.ExtractOpts{Cookies: jar})
	...
	if !strings.Contains(got, "https://media.example.com/zhengbao/lesson-1.mp4") {
